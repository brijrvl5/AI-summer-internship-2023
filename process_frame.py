...
class ProcessFrame:
    ...
    def _get_state(self, knee_angle):
        ...
        if (
            self.thresholds["HIP_KNEE_VERT"]["NORMAL"][0]
            <= knee_angle
            <= self.thresholds["HIP_KNEE_VERT"]["NORMAL"][1]
        ):
            knee = 1
        elif (
            self.thresholds["HIP_KNEE_VERT"]["TRANS"][0]
            <= knee_angle
            <= self.thresholds["HIP_KNEE_VERT"]["TRANS"][1]
        ):
            knee = 2
        elif (
            self.thresholds["HIP_KNEE_VERT"]["PASS"][0]
            <= knee_angle
            <= self.thresholds["HIP_KNEE_VERT"]["PASS"][1]
        ):
            knee = 3
    ...
    def _update_state_sequence(self, state):
        if state == "s2":
            if (
                ("s3" not in self.state_tracker["state_seq"])
                and (self.state_tracker["state_seq"].count("s2")) == 0
            ) or (
                ("s3" in self.state_tracker["state_seq"])
                and (self.state_tracker["state_seq"].count("s2") == 1)
            ):
                self.state_tracker["state_seq"].append(state)

        elif state == "s3":
            if (
                state not in self.state_tracker["state_seq"]
            ) and "s2" in self.state_tracker["state_seq"]:
                self.state_tracker["state_seq"].append(state)
        ...
        if keypoints.pose_landmarks:
            ...
            if offset_angle > self.thresholds["OFFSET_THRESH"]:
                ...
                self.state_tracker["INACTIVE_TIME_FRONT"] += (
                    end_time - self.state_tracker["start_inactive_time_front"]
                )
            ...
            # Camera is aligned properly.
                ...
                draw_dotted_line(
                    frame,
                    ankle_coord,
                    start=ankle_coord[1] - 50,
                    end=ankle_coord[1] + 20,
                    line_color=self.COLORS["blue"],
                )
                ...
                if current_state == "s1":
                    if (
                        len(self.state_tracker["state_seq"]) == 3
                        ...
                        and len(self.state_tracker["state_seq"]) == 1
                    ):
                ...
                # -------------------------------------- PERFORM FEEDBACK ACTIONS --------------------------------------
                    ...
                    if hip_vertical_angle > self.thresholds["HIP_THRESH"][1]:
                        ...
                        hip_vertical_angle < self.thresholds["HIP_THRESH"][0]
                        and self.state_tracker["state_seq"].count("s2") == 1
                    ):
                        self.state_tracker["DISPLAY_TEXT"][1] = True

                    if (
                        self.thresholds["KNEE_THRESH"][0]
                        < knee_vertical_angle
                        < self.thresholds["KNEE_THRESH"][1]
                        and self.state_tracker["state_seq"].count("s2") == 1
                    ):
                ...
                if self.state_tracker["curr_state"] == self.state_tracker["prev_state"]:
                    ...
                    self.state_tracker["INACTIVE_TIME"] += (
                        end_time - self.state_tracker["start_inactive_time"]
                    )
                ...
                cv2.putText(
                    frame,
                    str(int(hip_vertical_angle)),
                    (hip_text_coord_x, hip_coord[1]),
                    self.font,
                    0.6,
                ...
                cv2.putText(
                    frame,
                    str(int(knee_vertical_angle)),
                    (knee_text_coord_x, knee_coord[1] + 10),
                    self.font,
                    0.6,
                    self.COLORS["light_green"],
                    2,
                ...
                cv2.putText(
                    frame,
                    str(int(ankle_vertical_angle)),
                    (ankle_text_coord_x, ankle_coord[1]),
                    self.font,
                    0.6,
                    self.COLORS["light_green"],
                    2,
